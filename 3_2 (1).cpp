...
void *threadFunc(void *param)
{
        ...
        int bei_first_1_loc=p->bei_first_1_loc;
        int t_id=p->t_id;
        int i=p->i;
        bool judge1=p->judge1;
        bool judge2=p->judge2;
    
        for(int j=t_id;j<bin_a_v.size();j+=worker_count)
        {
            //在将原来的被消元行变为消元子时出错
...
int main()
{
    ...
    for (int i = 0; i < bin_b_v.size(); i++)
    {
        ...
        for(int t_id=0;t_id<worker_count;t_id++)
        {
                ...
                param[t_id].judge1=judge1;
                param[t_id].judge2=judge2;
                param[t_id].bei_first_1_loc=bei_first_1_loc;
                param[t_id].i=i;
        }
...
//     int j;
//     for(j=0;j+16<bin_col_num;j+=16)
//     {
//         __m512i v_k1=_mm512_loadu_si512((__m512i*)(k1+j));
//         __m512i v_k2=_mm512_loadu_si512((__m512i*)(k2+j));
//         //对位异或
//         v_k2=_mm512_xor_si512(v_k1,v_k2);
//         //将数据从向量寄存器存储到内存
//         _mm512_storeu_si512((__m512i*)(k2+j),v_k2);
//     }
//     for(;j<bin_col_num;j++)
//         k2[j]=k1[j]^k2[j];
